...
namespace LDDC.DAL
{
	...
	public partial class HotDishes
	{
		...
		public bool Exists(Guid guid)
		{
			...
			strSql.Append("select count(1) from HotDishes");
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(LDDC.Model.HotDishes model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into HotDishes(");
			strSql.Append("guid,rGuid,vGuid,creatorguid,creator,createDate,isValid)");
			...
			parameters[2].Value = model.vGuid;
			parameters[3].Value = model.creatorguid;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(LDDC.Model.HotDishes model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update HotDishes set ");
			...
			parameters[2].Value = model.creatorguid;
		...
		public bool Delete(Guid guid)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from HotDishes ");
			strSql.Append(" where guid=@guid ");
			SqlParameter[] parameters = {
					new SqlParameter("@guid", SqlDbType.UniqueIdentifier,16)			};
			parameters[0].Value = guid;
		...
		/// 得到一个对象实体
		/// </summary>
		public LDDC.Model.HotDishes GetModel(Guid guid)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 guid,rGuid,vGuid,creatorguid,creator,createDate,isValid from HotDishes ");
			strSql.Append(" where guid=@guid ");
			SqlParameter[] parameters = {
					new SqlParameter("@guid", SqlDbType.UniqueIdentifier,16)			};
			parameters[0].Value = guid;

			LDDC.Model.HotDishes model=new LDDC.Model.HotDishes();
		...
		/// 得到一个对象实体
		/// </summary>
		public LDDC.Model.HotDishes DataRowToModel(DataRow row)
		{
			LDDC.Model.HotDishes model=new LDDC.Model.HotDishes();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select guid,rGuid,vGuid,creatorguid,creator,createDate,isValid ");
			strSql.Append(" FROM HotDishes ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from HotDishes T ");
