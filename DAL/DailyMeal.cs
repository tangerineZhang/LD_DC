...
namespace LDDC.DAL
{
	...
	public partial class DailyMeal
	{
		...
		public bool Exists(Guid guid)
		{
			...
			strSql.Append("select count(1) from DailyMeal");
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(LDDC.Model.DailyMeal model)
		{
			StringBuilder strSql=new StringBuilder();
			...
			parameters[1].Value = model.sGuid;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(LDDC.Model.DailyMeal model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update DailyMeal set ");
		...
		public bool Delete(Guid guid)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from DailyMeal ");
			strSql.Append(" where guid=@guid ");
			SqlParameter[] parameters = {
					new SqlParameter("@guid", SqlDbType.UniqueIdentifier,16)			};
			parameters[0].Value = guid;
		...
		/// 得到一个对象实体
		/// </summary>
		public LDDC.Model.DailyMeal GetModel(Guid guid)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 guid,sGuid,vGuid,creatorguid,creator,createDate,isValid from DailyMeal ");
			strSql.Append(" where guid=@guid ");
			SqlParameter[] parameters = {
					new SqlParameter("@guid", SqlDbType.UniqueIdentifier,16)			};
			parameters[0].Value = guid;

			LDDC.Model.DailyMeal model=new LDDC.Model.DailyMeal();
		...
		/// 得到一个对象实体
		/// </summary>
		public LDDC.Model.DailyMeal DataRowToModel(DataRow row)
		{
			LDDC.Model.DailyMeal model=new LDDC.Model.DailyMeal();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select guid,sGuid,vGuid,creatorguid,creator,createDate,isValid ");
			strSql.Append(" FROM DailyMeal ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from DailyMeal T ");
