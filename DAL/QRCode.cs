...
namespace LDDC.DAL
{
	...
	public partial class QRCode
	{
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(LDDC.Model.QRCode model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into QRCode(");
			strSql.Append("guid,rGuid,code,createDate,isValid)");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(LDDC.Model.QRCode model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 得到一个对象实体
		/// </summary>
		public LDDC.Model.QRCode GetModel(Guid guid)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 guid,rGuid,code,createDate,isValid from QRCode ");
			strSql.Append(" where guid=@guid ");
			SqlParameter[] parameters = {
					new SqlParameter("@guid", SqlDbType.UniqueIdentifier,16)			};
			parameters[0].Value = guid;

			LDDC.Model.QRCode model=new LDDC.Model.QRCode();
		...
		/// 得到一个对象实体
		/// </summary>
		public LDDC.Model.QRCode DataRowToModel(DataRow row)
		{
			LDDC.Model.QRCode model=new LDDC.Model.QRCode();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select guid,rGuid,code,createDate,isValid ");
