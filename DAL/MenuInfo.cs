...
namespace LDDC.DAL
{
	...
	public partial class MenuInfo
	{
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(LDDC.Model.MenuInfo model)
		{
			StringBuilder strSql=new StringBuilder();
			...
			parameters[2].Value = model.vtID;
			parameters[3].Value = model.vtName;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(LDDC.Model.MenuInfo model)
		{
			StringBuilder strSql=new StringBuilder();
			...
			parameters[1].Value = model.vtID;
			parameters[2].Value = model.vtName;
		...
		/// 得到一个对象实体
		/// </summary>
		public LDDC.Model.MenuInfo GetModel(Guid guid)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 guid,rGuid,vtID,vtName,name,describe,creatorguid,creator,createDate,isValid from MenuInfo ");
			strSql.Append(" where guid=@guid ");
			SqlParameter[] parameters = {
					new SqlParameter("@guid", SqlDbType.UniqueIdentifier,16)			};
			parameters[0].Value = guid;

			LDDC.Model.MenuInfo model=new LDDC.Model.MenuInfo();
		...
		/// 得到一个对象实体
		/// </summary>
		public LDDC.Model.MenuInfo DataRowToModel(DataRow row)
		{
			LDDC.Model.MenuInfo model=new LDDC.Model.MenuInfo();
			if (row != null)
			{
				...
				{
					model.vtID=int.Parse(row["vtID"].ToString());
				}
				if(row["vtName"]!=null)
				{
					model.vtName=row["vtName"].ToString();
				}
				if(row["name"]!=null)
				{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select guid,rGuid,vtID,vtName,name,describe,creatorguid,creator,createDate,isValid ");
